...
o2-dpl-raw-proxy -b --session default \
  ...
  | o2-fv0-flp-dpl-workflow -b --session default --output-dir=/tmp --nevents ${NTF_TO_STORE} --pipeline  fv0-datareader-dpl:$N_PIPELINES --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
  | o2-datasampling-standalone -b --session default --config ${DS_GEN_CONFIG_PATH} \
  | o2-calibration-fv0-tf-processor -b --session default --dispatcher-mode --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
  | o2-calibration-fv0-channel-offset-calibration -b --session default --tf-per-slot 56000 --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
