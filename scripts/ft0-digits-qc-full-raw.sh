...
# Variables
WF_NAME=ft0-digits-qc-full-raw
...
DS_FINAL_CONFIG_PATH='consul-json://{{ consul_endpoint }}/o2/components/qc/ANY/any/ft0-digits-datasampling-{{ it }}'
...
o2-dpl-raw-proxy -b --session default \
  --dataspec 'rawdata:FT0/RAWDATA;dd:FLP/DISTSUBTIMEFRAME/0' \
  ...
  | o2-ft0-flp-dpl-workflow -b --session default --output-dir=/tmp --nevents ${NTF_TO_STORE} --pipeline  ft0-datareader-dpl:$N_PIPELINES --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
  | o2-datasampling-standalone -b --session default --config ${DS_GEN_CONFIG_PATH} \
  | o2-calibration-ft0-tf-processor -b --session default --dispatcher-mode --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
  | o2-calibration-ft0-channel-offset-calibration -b --session default --tf-per-slot 56000 --configKeyValues "${DPL_PROCESSING_CONFIG_KEY_VALUES}" \
  | o2-calibration-ccdb-populator-workflow -b --session default --ccdb-path=http://ccdb-test.cern.ch:8080 \
  | o2-dpl-output-proxy -b --session default --dataspec 'rawdata:FT0/RAWDATA;digits:FT0/DIGITSBC/0;channels:FT0/DIGITSCH/0;dd:FLP/DISTSUBTIMEFRAME/0' \
